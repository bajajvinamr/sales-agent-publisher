...
- **Tunnel uses `--protocol http2`, not QUIC.** Default QUIC is fragile on Bangalore (`blr02`) + Mumbai (`bom09`) edge POPs and was the failure mode in 100% of flap logs. Do not remove the flag from `docker-compose.yml`.
- **Never restart the tunnel on app-only deploys.** `deploy.yml` should ideally rebuild only `app` (`docker compose up -d --build app`) unless `docker-compose.yml` or `cloudflared.yml` changed. A full `up -d --build` triggers a tunnel restart and a 5–15 min Cloudflare edge propagation window of false-positive 503s. Phase 2a of the tunnel-flap plan implements this guard.
